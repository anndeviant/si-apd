...
CREATE TABLE public.pegawai (
  ...
  CONSTRAINT pegawai_bengkel_id_fkey FOREIGN KEY (bengkel_id) REFERENCES public.apd_bengkel(id)
);
CREATE TABLE public.pengajuan_apd (
  id bigint GENERATED ALWAYS AS IDENTITY NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  nama_project text,
  nomor_project text,
  kepala_project text,
  progres text DEFAULT 'Draft'::text,
  keterangan text,
  tanggal date,
  apd_nama text,
  jumlah bigint,
  ...
  harga numeric,
  total numeric DEFAULT ((jumlah)::numeric * harga),
  CONSTRAINT pengajuan_apd_pkey PRIMARY KEY (id)
);
